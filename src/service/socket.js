    ...
    switch (event) {
      ...
      case "MATCH:UPDATE":
        if ( this.matchId == data.id ) {
          ...
          let { activePlayerName } = server
          let activePlayer
          if(data.player2){
            if ( activePlayerName === server.match.player1.name ) activePlayer = data.player1
            if ( activePlayerName === server.match.player2.name ) activePlayer = data.player2
          }

          // console.log( "UPDATE ACTIVEPLAYER", { activePlayer } )
          useServer.setState( () => ({ activePlayer, match: data }) )
        } else {
